...
def test_quality_mode_uses_guided_pipeline_for_simple_target():
    router = GroundingRouter(
        RoutingPolicy(yolo_target_aliases={"person": ["man"]})
    )
    item = request("find the person", "person").model_copy(
        update={"performance_mode": PerformanceMode.QUALITY}
    )
    decision = router.route(item, health("yolo", "owlvit", "gpt_guided_owlvit"))
    assert decision.selected_backend == "gpt_guided_owlvit"


def test_fast_mode_keeps_guided_instruction_on_local_backend():
    router = GroundingRouter(
        RoutingPolicy(yolo_target_aliases={"person": ["man"]})
    )
    item = request("find the person near the table", "person").model_copy(
