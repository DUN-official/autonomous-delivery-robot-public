...
def test_quality_mode_runs_selection_refinement_and_edge_review(tmp_path):
    ...
    owl = _Owl(
        [
            [_candidate("owl_1", [20, 20, 80, 90]), _candidate("owl_2", [100, 20, 160, 90])],
            [_candidate("local_1", [5, 5, 55, 65]), _candidate("local_2", [8, 8, 58, 68])],
        ]
    )
    ...
    backend._client = _Client(
        [
            {"status": "selected", "selected_candidate": 1, "confidence": 0.9, "reason": "initial"},
            {"status": "selected", "selected_candidate": 2, "confidence": 0.92, "reason": "refined"},
...
def test_quality_mode_can_recover_from_a_coarse_region(tmp_path):
    ...
    backend._client = _Client(
        [
            {"status": "refine", "region": [0.2, 0.2, 0.7, 0.8], "confidence": 0.8, "reason": "coarse"},
            {"status": "selected", "selected_candidate": 1, "confidence": 0.9, "reason": "refined"},
            {"decision": "accept", "confidence": 0.95, "reason": "complete box"},
        ]
    )
