...
def serve():
    ...
    os.environ["GROUNDING_CONFIG"] = str(config_path)

    # Keep Ultralytics state inside the project instead of writing to the
    # user's roaming profile. An explicit environment override still wins.
    if "YOLO_CONFIG_DIR" not in os.environ:
        raw_config = json.loads(config_path.read_text(encoding="utf-8"))
        project_root_setting = raw_config.get("service", {}).get("project_root", "..")
        project_root = (config_path.parent / project_root_setting).resolve()
        yolo_config_dir = project_root / "runtime" / "ultralytics"
...
def ground_image():
    ...
    maximum_latency_ms = args.maximum_latency_ms or latency_by_mode[args.mode]
    ...
    data = {
        ...
        "maximum_latency_ms": str(maximum_latency_ms),
    }
    ...
    with image_path.open("rb") as handle:
        response = httpx.post(
            ...
            timeout=max(30.0, maximum_latency_ms / 1000.0 + 5.0),
        )
