...
class GroundingDINOSettings(BackendBaseSettings):
    model_path: str = "models/grounding_dino/grounding-dino-tiny"
    device: str = "auto"
    thresholds: list[float] = Field(default_factory=lambda: [0.08, 0.05, 0.02, 0.005])
    top_k: int = 6
    max_pairwise_iou: float = 0.85
...
class GPTVisionSettings(BackendBaseSettings):
...
class BackendSettings(BaseModel):
    ...
    gpt_vision: GPTVisionSettings = Field(default_factory=GPTVisionSettings)
