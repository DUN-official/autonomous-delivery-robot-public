...
class LocalGroundingService:
    ...
    def from_config(cls, config: GroundingConfig) -> "LocalGroundingService":
        ...
        settings = config.backends.grounding_dino
        if settings.enabled:
            backends["grounding_dino"] = GroundingDINOBackend(
        ...
        settings = config.backends.gpt_vision
        if settings.enabled:
            backends["gpt_vision"] = GPTVisionBackend(
