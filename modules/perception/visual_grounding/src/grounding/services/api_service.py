...
ALLOWED_VIDEO_BACKENDS = {"auto", "yolo", "grounding_dino", "owlvit", "gpt_vision", "gpt_guided_owlvit"}
VIDEO_MAX_BYTES = 1024 * 1024 * 1024
PROFILE_LATENCY_MS = {
    PerformanceMode.FAST: 120_000,
    PerformanceMode.BALANCED: 300_000,
    PerformanceMode.QUALITY: 600_000,
    PerformanceMode.ACCURATE: 600_000,
}
QUALITY_MODES = {PerformanceMode.QUALITY, PerformanceMode.ACCURATE}
...
def profile_latency_ms(
    ...
    return (
        ...
        else PROFILE_LATENCY_MS[performance_mode]
    )
...
def validate_profile_backend(
    ...
    if performance_mode in QUALITY_MODES and normalized not in {
        "",
        "auto",
        "gpt_guided_owlvit",
    }:
        raise HTTPException(
            status_code=400,
            detail="Quality profile requires Auto or GPT-guided OWL-ViT",
        )
...
def create_app(config_path: str | Path | None = None):
    ...
    app = FastAPI(
        title="Adaptive Visual Grounding Service",
        version="0.6.0",
        lifespan=lifespan,
    )
    ...
    async def ground_upload(
        ...
        performance_mode: PerformanceMode = Form(default=PerformanceMode.QUALITY),
        maximum_results: int | None = Form(default=None),
        maximum_latency_ms: int | None = Form(default=None),
        preferred_backend: str | None = Form(default=None),
        parser_mode: str = Form(default="llm"),
        return_segmentation: bool = Form(default=False),
    ):
        validate_profile_backend(performance_mode, preferred_backend)
        ...
        request_data: dict[str, object] = {
            ...
            "action": action,
            "performance_mode": performance_mode,
            "maximum_latency_ms": profile_latency_ms(
                performance_mode,
                maximum_latency_ms,
            ),
    ...
    async def video_upload(
        ...
        validate_profile_backend(performance_mode, backend)
        ...
        session_id = get_video_manager().create(
            ...
            maximum_latency_ms=profile_latency_ms(performance_mode),
