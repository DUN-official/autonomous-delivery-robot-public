...
from ..schemas import (
    ...
    GroundingStatus, PerformanceMode, QuantityIntent, TraceEvent,
)
...
class GPTGuidedOWLViTBackend(GroundingBackend):
    ...
    def __init__(
        ...
        api_key_env="OPENAI_API_KEY", image_detail="high",
        ...
        openai_timeout_seconds=120.0, openai_max_retries=1,
    ...
    def startup(self):
        ...
        yolo_state = "ready" if self.yolo_backend and self.yolo_backend.health().loaded else "unavailable"
        self._started = True
        self._health_detail = (
            "fast, balanced, and quality grounding paths ready; "
            f"YOLO fast path {yolo_state}"
        )
